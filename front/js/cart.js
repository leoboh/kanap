 ...
 */
fetch("https://kanap-back-two.vercel.app/api/products")
   .then((resp) => resp.json())
   .then(function (dataApi) {
      ...
      for (let produit of stockProduit) {
         ...
         /**
          * Calcule des totaux sur l'input qui gère la quantité
          */
...
inputFirstName.addEventListener("input", () => {
   ...
   if (firstNameIsValid) {
      ...
      errorFirstName.textContent = "Le prénom que vous avez saisi n'est pas recevable, il ne doit contenir que des lettres";
      return false;
   }
});
...
inputLastName.addEventListener("input", () => {
   ...
   if (lastNameIsValid) {
      ...
      errorLastName.textContent = "Le nom que vous avez saisi n'est pas recevable.";
   }
});
...
inputAdress.addEventListener("input", () => {
   ...
   if (adressIsValid) {
      ...
      errorAdress.textContent = "L'adresse que vous avez saisi n'est pas recevable";
   }
});
...
inputEmail.addEventListener("input", () => {
   ...
   if (emailIsValid) {
      ...
      errorEmail.textContent = "L'adresse que vous avez saisi n'est pas recevable";
   }
});
...
form.addEventListener("submit", function (e) {
   ...
   if (formIsValid) {
      ...
      // Recupérer la qtt des produits et instancier un objet pour la qtt recupérer même s'ils ont la même id (couleur différente)
      let products = [];
       ...
       */
      fetch("https://kanap-back-two.vercel.app/api/products/order", optionOrder)
         .then((resp) => resp.json())
         .then(function (orderId) {
            // session storage pour pouvoir recuperer donnee dans confirm
            sessionStorage.setItem("orderId", orderId.orderId);
            window.location.href = "./confirmation.html?id=" + orderId.orderId;
         });
   } else {
      alert("Le formulaire est incomplet, veuillez vérfier que tous les champs soient bien remplis");
   }
});
