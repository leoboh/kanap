 ...
 */
fetch("https://kanap-back-two.vercel.app/api/products/" + idUrl)
   .then((resp) => resp.json())
   .then(function (produitUrl) {
      ...
      imgProduit.alt = produitUrl.altTxt;
      ...
      btnCommander.addEventListener("click", (e) => {
         ...
         let v = optionProduit.qttProduit > 0 && optionProduit.qttProduit <= 100;
               ...
               if (stockProduit) {
                  ...
                  for (produits of stockProduit) {
                     produitDejaStocker = stockProduit.find(
                        (a) => a.idProduit === optionProduit.idProduit && a.colorProduit === optionProduit.colorProduit
                     );
                  }

                  if (produitDejaStocker) {
                     // si un produit similaire a été trouvé
                     // on push la nouvelle quantité produit
                     produitDejaStocker.qttProduit = Number(produitDejaStocker.qttProduit) + Number(optionProduit.qttProduit);
                     // ajouter qtt stck a option
